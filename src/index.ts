...
class Vuelidate extends Vue {
  ...
  constructor (validations: Declarations, vmObj: object) {
...
function setupValidations (validations: Declarations, vm: Vue): object {
  ...
  const validationResults = modelKeys.reduce(constructResults, { $invalid: false })
  ...
  function constructResults (results: Results, model: string) {
    ...
    results.$invalid = Object.keys(results).some(rule => results[rule].$invalid)
    return results
  }
}
...
function validateModel (rules: Rules, value: any): Results {
  ...
  return ruleKeys.reduce((results: Results, ruleKey: string): Results => {
...
function mapDynamicKeyName (k): string { return 'v$$' + k }

function buildDynamics (obj, fn): object {
  return reduceObj(obj, (build, val, key) => {
